...
- **GREEN blocks are executable as written** — no `...`/TBD placeholders and no deferral notes ("verify the signature during implementation")
- **Cited callee signatures actually match the real code.** Where a GREEN block calls into existing code, open the cited file and confirm the signature. This check lives here rather than as a planning obligation because verifying every callee while drafting would push a large amount of exploration into planning; a reviewer can spot-check the calls that matter for far less.
- **Constraints below the application layer are respected** — SQL triggers, `CHECK` constraints, and immutability rules defined in migrations, not just Pydantic or application-level invariants
- **No wall-clock-dependent literals** in RED tests
...
>
> Then check that the plan's embedded code could actually run. Flag as "Must Address" any GREEN block containing `...`, TBD, or a note deferring design to implementation time — that is exploratory coding disguised as a plan. Where a GREEN block calls existing code, open the cited file and confirm the signature matches; a plan that calls `resolve(...)` with a literal ellipsis and a note to "verify the exact signature during implementation" has not been planned. Trace at least the highest-risk GREEN block against its own RED test and confirm it would pass — one plan shipped GREEN code that failed the very test the plan embedded. Finally, check constraints the application layer cannot see: a migration-defined trigger such as `immutable_terminal_run` will block a literal snippet that a Pydantic-level review passes without comment.
...
4. **Mode-specific behavior**:
   - **Non-interactive mode**: spawn the multi-persona reviewer sub-agents (mandatory — self-review is NEVER a substitute for sub-agent spawning, even in `--non-interactive` mode). Run a single iteration only. Auto-resolve "Must Address" items by applying fixes directly; note "Should Consider" items as advisory comments in the plan. Do not prompt for input. Token cost is NOT a valid skip justification.
