...
### pytest-flavored example: Align Assertions with Test Name

Before (misaligned — name promises an exception, asserts on return value):
...
After (aligned — verifies the promised exception with `pytest.raises`):
