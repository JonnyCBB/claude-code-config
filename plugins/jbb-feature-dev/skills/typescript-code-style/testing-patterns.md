...
The canonical test-review patterns (`TestNameAssertionMismatch`, `VagueNaming`, `MissingContext`, `MissingFeatureAssertion`) are defined once in `${CLAUDE_PLUGIN_ROOT}/skills/code-style-common/test-anti-patterns.md` to maintain Skill DRY (per Reqs Success Criterion line 126). See that file for full Before/After examples across all four patterns.

### RTL/@testing-library example: Align Assertions with Test Name

Before (misaligned — name promises an error toast, asserts on button state):
...
After (aligned — uses RTL's `findByRole` to verify the promised error toast):
