...
### Pattern: Enforce Exhaustiveness with `never`

A `switch` over a discriminated union is only _implicitly_ exhaustive when the function's return type forces it. To make exhaustiveness a hard compile error regardless of return position, add a `default` branch that assigns the discriminant to `never`. Adding a new variant to the union then produces a TypeScript error at every match site that forgot to handle it.

Before:

```typescript
function label(g: GroupType): string {
  switch (g.kind) {
    case "entity":
      return "entity";
    case "carousel":
      return "carousel";
    default:
      return "unknown"; // silently absorbs new variants
  }
}
```
...
function label(g: GroupType): string {
  switch (g.kind) {
    ...
    case "instant_mix":
      return "instant_mix";
    default: {
      const _exhaustive: never = g; // compile error if a variant is added
      throw new Error(`Unhandled GroupType: ${JSON.stringify(_exhaustive)}`);
    }
  }
}
```

Avoid a silent `default: return 'unknown'` (or `return null`) — it defeats the exhaustiveness check that discriminated unions exist to provide. See `code-style-common/structural-patterns.md` for the cross-language rationale.
