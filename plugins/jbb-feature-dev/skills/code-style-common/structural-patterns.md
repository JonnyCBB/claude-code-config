...
Methods should be ~20 lines or less. If longer, extract sub-methods. (Python applies a deliberately stricter ~10-line convention; see `python-code-style/SKILL.md`.)
...
When pattern-matching on a closed set (enum, sealed type, discriminated union), enumerate every case rather than falling back to a default/wildcard branch. The compiler can then flag the match site when a new variant is added, turning a silent runtime regression into a build error.

Use a default/wildcard branch only when:

1. The input is genuinely open (arbitrary `String`, untyped JSON, etc.), or
2. The catch-all `throw`s / `raise`s / calls `assert_never` — that is the recommended form, not an anti-pattern.

Before (Java, but the pattern is language-agnostic):

```
return switch (type) {
  case ENTITY -> GroupType.ENTITY;
  case CAROUSEL -> GroupType.CAROUSEL;
  default -> null;        // silently swallows new variants
};
```
...
return switch (type) {
  case ENTITY -> GroupType.ENTITY;
  case CAROUSEL -> GroupType.CAROUSEL;
  case INSTANT_MIX -> GroupType.INSTANT_MIX;
  // no default — adding a new variant is now a compile error
};
```

If "unknown" really is a valid runtime outcome, prefer an explicit case that throws (e.g. `case UNKNOWN -> throw new IllegalStateException(...)`) over a silent `default -> null`.

**Language-specific mechanisms**:

- **Java**: switch expressions on enums and sealed types are exhaustive when `default` is omitted. See `java-code-style/modern-java.md`.
- **Scala**: `match` on a sealed trait/enum is exhaustive when the wildcard `case _ =>` is omitted; do not silence the warning with `@nowarn` or `@unchecked`.
- **TypeScript**: discriminated unions are exhaustive when the `default` branch assigns the discriminant to `never` (`const _exhaustive: never = x;`). See `typescript-code-style/react-patterns.md`.
- **Python**: in `match` over an `Enum` or `Literal`, use `case _: assert_never(x)` (`typing.assert_never`, 3.11+) so mypy/pyright flag unhandled variants.

**When to flag**: a switch/match expression over an enum, sealed type, ADT, or discriminated union that uses `default`, `case _`, or equivalent to return a fallback value. Exception: if the catch-all `throw`s / `raise`s / calls `assert_never`, that is the recommended form.
