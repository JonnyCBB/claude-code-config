...
This template defines the output format for Phase 4 (Document) of the improve-codebase-architecture
...
proposal to `~/.claude/thoughts/shared/improve-codebase-architecture-proposals/YYYY-MM-DD-description.md`.
...
Three short bullets that orient downstream agents acting on this proposal. Each bullet should be specific to the modules and files this refactor touches.

- **Always do**: invariants, conventions, or guardrails the refactor must preserve (e.g., "all storage access goes through the new `UserDataPort`").
- **Ask first**: changes that need maintainer sign-off before proceeding (e.g., "renaming the public `getRecommendations` API; deleting the old `RecommendationService`").
- **Never touch**: files or contracts out of scope for this refactor (e.g., "callers outside `src/services/recommendation/`; the `/v1/recommendations` HTTP contract").
