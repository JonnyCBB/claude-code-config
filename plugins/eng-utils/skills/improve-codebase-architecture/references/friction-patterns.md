...
2. [Friction Patterns](#friction-patterns)
   ...
   9. [The Deletion Test](#the-deletion-test)
...
**Sub-pattern — Complecting (Hickey).** Rich Hickey's term from _Simple Made Easy_ — from Latin _complectere_, "to braid together." Complecting names the act of interleaving concerns that could remain separate. State complects time; methods complect function-and-state; inheritance complects types. Look for complecting when the cyclic dependency is structural rather than incidental: two modules cannot be disentangled because their concepts have been braided rather than composed.
...
Before recommending that a module be merged, inlined, or eliminated, apply the deletion test (adapted from Matt Pocock's `improve-codebase-architecture` skill):

> _Imagine deleting the module. If complexity vanishes, the module wasn't hiding anything (it was a pass-through). If complexity reappears across N callers, the module was earning its keep._

Use this as a sanity check on every "shallow module" or "leaky abstraction" finding: a module that fails the deletion test is a refactor candidate; one that passes is genuinely deep, even if its line-count ratio looks unflattering.
...
**Empirical grounding:** Buehler et al. (arXiv 2604.13108, "Formal Architecture Descriptors as Navigation Primitives for AI Coding Agents") demonstrate that these metrics directly predict AI agent task success — agents given structured architecture descriptors with files-per-concept clustering, context budget constraints, and module depth annotations complete significantly more tasks than agents navigating the same codebases without them. The paper validates that codebase navigability is not just a developer experience concern but a measurable input to agent performance.
...
- **Interpretation:** Higher depth means the module absorbs more complexity behind a simple interface. Low-depth modules (shallow modules) push complexity onto their callers and force agents to understand more code to accomplish tasks. Note: this metric can be gamed by padding the implementation with verbose private helpers. Use it as one signal among several, not as a sole criterion. Pair it with the Deletion Test (above) before recommending a refactor.
