...
- [Category 3: Remote but Owned (Ports & Adapters)](#category-3-remote-but-owned-ports--adapters)
  - [The Two-Adapter Rule](#the-two-adapter-rule)
...
- Examples: PGLite for Postgres, SQLite for relational queries, in-memory filesystem, an in-memory wide-column store, embedded Redis, testcontainers
...
// Test (using PGLite / an in-memory store / embedded store)
...
An in-memory mode of your datastore lets you test wide-column read/write logic without a remote cluster:
...
- Examples: calls to internal gRPC services, Kafka consumers/producers, Hermes pub/sub
...
Before introducing a port, ask whether the codebase will have at least two real adapters implementing it (e.g., one in-memory test adapter and one production gRPC/HTTP adapter, or two production transports). Pocock's heuristic:

> _One adapter means a hypothetical seam. Two adapters means a real seam._

A port with only ever one adapter adds indirection without earning seam value. Defer the port until a second implementation is concretely needed — including the in-memory test adapter, which by itself qualifies as the second.
...
A deepened module can still have **internal seams** — private boundaries inside its implementation, used by its own tests — that are distinct from its **external seam** at the public interface. Internal seams are fine; they are tools the module uses to test itself. The discipline is to assert through the external seam in public contracts and to keep internal-seam tests labelled and scoped so they can be deleted without destabilizing the public test suite.
...
| Local-substitutable | Local I/O | Real local stand-in | Start stand-in in test setup | In-memory store, PGLite     |
